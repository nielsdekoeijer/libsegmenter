...
def hamming(windowLength):
    ...
    M = np.float64(windowLength)
    alpha = 25.0/46.0
    beta = (1-alpha)/2.0
    window = alpha - 2*beta*np.cos(2.0*np.pi*np.arange(0,windowLength, dtype=np.float64)/windowLength)
    return window
...
def defaultWindowSelector(windowName, windowLength):
    match windowName:
        ...
        case "kaiser82":
            ...
            hopSize = int(np.floor(1.7*(np.float64(windowLength)-1.0)/(beta + 1.0)))
            return window, hopSize
        case "kaiser85":
        ...
        case "rectangular0":
            ...
            window = np.ones(windowLength, dtype=np.float64)
