...
def check_cola(window, hopSize, plot_flag=False):
    ...
    if plot_flag:
        ...
        normFrequency = np.arange(
            0, plotLength, dtype=np.float64) / np.float64(plotLength)
        ...
        ax.plot(1.0/np.float64(2.0*hopSize) *
                np.ones(2, dtype=np.float64), plotLim, dashes=[6, 2])
    ...
    for k in range(1, hopSize):
        ...
        if plot_flag:
            ax.plot(np.float64(k)/np.float64(hopSize)*np.ones(2,
                                                              dtype=np.float64), plotLim, dashes=[6, 2], color='k')
